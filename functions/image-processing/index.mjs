...
export const handler = async (event) => {
  ...
  try {
    ...
    return sendError(500, `Error downloading original image: ${originalImagePath}`, error);
  }
  var timingLog = 'img-download;dur=' + parseInt(performance.now() - startTime);
  let transformedImage;

  if (contentType === 'image/svg+xml') {
    // For SVGs, skip Sharp processing but still store in transformed bucket
    transformedImage = Buffer.from(originalImageBody);
    timingLog =
      timingLog +
      ',img-transform;dur=' +
      parseInt(performance.now() - startTime);
  } else {
    transformedImage = Sharp(await originalImageBody, {
      failOn: 'none',
      animated: true,
    });
