...
- **S3_TRANSFORMED_IMAGE_BUCKET_NAME** Recommended for using an existing S3 bucket where your transformed images are stored when deploying in production. Usage: cdk deploy -c S3_TRANSFORMED_IMAGE_BUCKET_NAME=’YOUR_S3_BUCKET_NAME’. Without specifiying this parameter, the stack creates a new S3 bucket for storing transformed images if STORE_TRANSFORMED_IMAGES=true
